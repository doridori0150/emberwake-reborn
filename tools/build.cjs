...
for (const a of Object.values(manifest.assets)) {
  if (!cache.has(a.src)) {
    ...
    bytes += buf.length;
    cache.set(a.src, 'data:image/' + path.extname(a.src).slice(1) + ';base64,' + buf.toString('base64'));
  }
...
html = html.replace(/<script src="([^"]+)"><\/script>/g, (m, src) =>
  src === 'src/asset-manifest.js'
    ? '<script>(function(g){g.ER=g.ER||{};g.ER.ASSETS=' + safe(JSON.stringify(manifest)) + ';})(globalThis);</script>'
    : '<script>\n' +
      safe(
        src === 'src/content.js'
          ? read(src).replace(/"src": "(assets\/portraits\/[a-z0-9_]+\.(png|jpg|webp))"/g, (m, f, ext) => {
              const buf = fs.readFileSync(path.join(ROOT, f));
              bytes += buf.length;
              return '"src": "data:image/' + (ext === 'jpg' ? 'jpeg' : ext) + ';base64,' + buf.toString('base64') + '"';
            })
          : read(src)
      ) +
      '\n</script>'
...
const body = html.slice(html.indexOf('<body>') + 6, html.indexOf('</body>')),
  style = html.slice(html.indexOf('<style>'), html.indexOf('</style>') + 8);
fs.writeFileSync(
  path.join(ROOT, 'dist', 'EmberwakeReborn-Artifact.html'),
  ['<title>Emberwake Reborn</title>', style, '<style>html,body{height:100%;background:#0e1113}</style>', body].join(String.fromCharCode(10))
); // claude.ai Artifact 게시용
console.log(
  '빌드 완료:',
  path.relative(ROOT, out),
  (fs.statSync(out).size / 1048576).toFixed(2) + ' MB',
  '(에셋 원본',
  (bytes / 1048576).toFixed(2) + ' MB, 파일',
  cache.size + '개)'
);
