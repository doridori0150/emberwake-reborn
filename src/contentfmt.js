...
(function (g) {
  ...
  function check(c) { // 저장 전 최소 모양 검사. 문제가 있으면 메시지를 돌려준다.
    ...
    for (const k of ['gear', 'craftOptions', 'portraits', 'npcs']) { if (c[k] && (typeof c[k] !== 'object' || Array.isArray(c[k]))) return k + ' 는 객체여야 한다'; for (const id of Object.keys(c[k] || {})) if (!/^[a-z][a-z0-9_.]*$/.test(id)) return k + ' id 는 영문 소문자·숫자·_ 만 쓴다: ' + id; }
    ...
    for (const [i, sp] of c.spawns.entries()) if (!sp || typeof sp.region !== 'string' || typeof sp.room !== 'string' || !Array.isArray(sp.group) || !sp.group.length || sp.group.some(k => typeof k !== 'string')) return '등장 조합 ' + (i + 1) + ': region·room·group(적 id 목록)이 필요하다';
    for (const [k, d] of Object.entries(c.enemies)) { if (!d || typeof d !== 'object' || typeof d.name !== 'string' || typeof d.asset !== 'string' || typeof d.ai !== 'string') return '적 ' + k + ': name·asset·ai 가 필요하다'; for (const f of ['hp', 'dmg', 'speed', 'detect', 'size']) if (!num(d[f]) || d[f] < 0) return '적 ' + k + ': ' + f + ' 는 0 이상의 수'; if (!Array.isArray(d.gold) || d.gold.length !== 2 || !d.gold.every(num)) return '적 ' + k + ': gold 는 [최소, 최대]'; if (!Array.isArray(d.loot)) return '적 ' + k + ': loot 는 배열'; }
    for (const [k, x] of Object.entries(c.gear || {})) { if (!x || typeof x.name !== 'string') return '장비 ' + k + ': name 이 필요하다'; for (const [m, n] of Object.entries(x.cost || {})) if (!num(n) || n < 1) return '장비 ' + k + ': 비용 ' + m + ' 는 1 이상'; if (x.weapon && typeof x.weapon.dice !== 'string') return '장비 ' + k + ': 무기 주사위가 필요하다'; }
    for (const [k, o] of Object.entries(c.craftOptions || {})) { if (!o || typeof o.name !== 'string' || !Array.isArray(o.slots)) return '제작 옵션 ' + k + ': name·slots 가 필요하다'; for (const n of Object.values(o.cost || {})) if (!num(n) || n < 1) return '제작 옵션 ' + k + ': 비용은 1 이상'; }
    for (const e of c.events || []) { if (!e.trigger || typeof e.trigger.type !== 'string' || !Array.isArray(e.pages)) return '이벤트 ' + e.id + ': trigger·pages 가 필요하다'; for (const ch of e.choices || []) { const r = ch.require || {}; if (r.gold !== undefined && (!num(r.gold) || r.gold < 0)) return '이벤트 ' + e.id + ': 조건 금화는 0 이상'; for (const n of Object.values(r.mat || {})) if (!num(n) || n < 1) return '이벤트 ' + e.id + ': 조건 재료는 1 이상'; for (const fx of ch.effects || []) if ('n' in fx && !num(fx.n)) return '이벤트 ' + e.id + ': 효과 수치가 숫자가 아니다'; } }
    for (const [k, n] of Object.entries(c.npcs || {})) if (!n || typeof n.name !== 'string' || !num(n.x) || !num(n.y) || !Array.isArray(n.greet)) return '주민 ' + k + ': name·x·y·greet 가 필요하다';
  ...
  function text(c) {
    ...
    dict('gear'); dict('craftOptions'); dict('portraits'); dict('npcs'); L.push('  "tuning": ' + spaced(c.tuning || {}) + ',');
