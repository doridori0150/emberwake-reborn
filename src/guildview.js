/* 길드 안뜰 화면. 폐허에서 시작해 시설 단계·총 복구도에 따라 설비·조명·장식·대원의 모습이 달라진다.
...
(function (g) {
  ...
  class GuildView {
    ...
    draw() {
      const ctx = this.ctx, Gs = this.G, now = performance.now(), total = this.total(); ctx.imageSmoothingEnabled = false; this.rects = {};
      ...
      // 대원: 맡은 자리 근처에 서 있다.
      const posts = { ara: [6.9, 4.2, 'down'], noa: Gs.facilities.workshop ? [5.6, 5.6, 'left'] : [9.6, 4.6, 'down'], lumi: [10.6, 9.0, 'right'] };
      for (const h of Gs.roster) { const [x, y, dir] = posts[h]; items.push({ hero: h, cx: x * T, by: y * T, dir }); }
      items.sort((a, b) => a.by - b.by);
      for (const it of items) {
        if (it.hero) { const a = gfx.asset(D.HEROES[it.hero].asset), work = it.hero === 'noa' && Gs.facilities.workshop && (now % 3200) < 330, frame = gfx.actorFrame(a, work ? 'attack' : 'idle', it.dir, work ? now % 3200 : now); ctx.fillStyle = 'rgba(0,0,0,.35)'; ctx.beginPath(); ctx.ellipse(it.cx, it.by, 20, 7, 0, 0, 7); ctx.fill(); gfx.sprite(ctx, D.HEROES[it.hero].asset, frame, it.cx, it.by - Math.sin(now / 450 + it.cx) * 1.2, 56); if (Gs.selected.hero === it.hero) this.tag(ctx, it.cx, it.by + 18, D.HEROES[it.hero].name + ' · 출격 대기', '#5fc9b8'); continue; }
