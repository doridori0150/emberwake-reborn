...
(function (g) {
  'use strict';
  const ER = g.ER = g.ER || {}, D = ER.data, esc = s => String(s ?? '').replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
  const CSS = '.erdlg{position:fixed;inset:0;z-index:60;display:flex;align-items:flex-end;justify-content:center;background:linear-gradient(transparent 35%,rgba(0,0,0,.72));padding:0 16px max(18px,env(safe-area-inset-bottom));font-family:inherit}' +
  ...
  function portrait(box, id) { // 초상화: 올린 그림(src)이 있으면 그것, 없으면 대원 스프라이트의 상반신
  ...
  function open(ev, o = {}) {
    close(); if (!document.getElementById('erdlg-css')) { const st = document.createElement('style'); st.id = 'erdlg-css'; st.textContent = CSS; document.head.append(st); }
    const root = document.createElement('div'); root.className = 'erdlg'; root.id = 'erdlg'; root.setAttribute('role', 'dialog'); root.setAttribute('aria-modal', 'true');
    root.innerHTML = '<div class="wrap"><div class="pt none"></div><div class="box"><div class="who"></div><div class="txt"></div><div class="ch"></div><div class="foot"><span class="cnt"></span><button class="next" type="button">다음 <kbd>Enter</kbd></button></div></div></div>'; document.body.append(root);
    const pages = (ev.pages || []).slice(), choices = ev.choices || [], fill = s => String(s || '').replace(/\{hero\}/g, o.vars?.hero || '대원'); let i = 0, reply = null, picked = -1;
    const $ = s => root.querySelector(s), done = () => { close(); if (o.onChoose) o.onChoose(picked); if (o.onClose) o.onClose(); };
    function show() {
      const last = pages[pages.length - 1] || {}, pg = reply ? { speaker: last.speaker, portrait: last.portrait, side: last.side, text: reply } : pages[i] || { text: '' };
      $('.wrap').className = 'wrap' + (pg.side === 'right' ? ' right' : ''); portrait($('.pt'), pg.portrait); $('.who').textContent = fill(pg.speaker); $('.who').hidden = !pg.speaker; $('.txt').textContent = fill(pg.text);
      const atEnd = !reply && i >= pages.length - 1, ask = atEnd && choices.length > 0; $('.cnt').textContent = (ev.name || '') + (pages.length > 1 && !reply ? '  ' + (i + 1) + '/' + pages.length : ''); $('.next').hidden = ask; $('.next').firstChild.textContent = reply || atEnd ? '닫기 ' : '다음 ';
      const ch = $('.ch'); ch.innerHTML = ''; if (ask) choices.forEach((c, k) => { const ok = o.can ? o.can(c) : true, b = document.createElement('button'); b.type = 'button'; b.disabled = !ok; const need = ER.events.requireText(c.require), fx = ER.events.effectText(c.effects);
        b.innerHTML = '<kbd>' + (k + 1) + '</kbd>' + esc(fill(c.label)) + (need ? '<span class="need">필요: ' + esc(need) + '</span>' : '') + (fx ? '<span class="fx">' + esc(fx) + '</span>' : ''); b.onclick = () => { picked = k; if (c.reply) { reply = c.reply; show(); } else done(); }; ch.append(b); });
      (ask ? ch.querySelector('button:not(:disabled)') : $('.next'))?.focus();
    }
    const advance = () => { if (reply) return done(); if (i < pages.length - 1) { i++; return show(); } if (!choices.length) return done(); };
    $('.next').onclick = advance;
    root.addEventListener('keydown', e => { e.stopPropagation(); if (e.key === 'Enter' || e.key === ' ') { if (document.activeElement?.closest('.ch')) return; e.preventDefault(); advance(); } else if (/^[1-9]$/.test(e.key)) { const b = root.querySelectorAll('.ch button')[+e.key - 1]; if (b && !b.disabled) b.click(); } });
    show(); return root;
  }
  function close() { document.getElementById('erdlg')?.remove(); }
