...
(function (g) {
  ...
  function seedStreams(seed) { const s = {}; for (const n of STREAMS) s[n] = hash(seed + ':' + n) || 1; return s; }
  ...
  function shuffle(state, stream, list) { const a = list.slice(); for (let i = a.length - 1; i > 0; i--) { const j = int(state, stream, i + 1); [a[i], a[j]] = [a[j], a[i]]; } return a; }

  function parse(expr) {
    if (typeof expr === 'number') { if (!Number.isSafeInteger(expr)) throw new TypeError('고정값 오류'); return { formula: String(expr), count: 0, sides: 1, mul: 1, mod: expr }; }
    ...
    const count = +m[1], sides = +m[2], mul = +(m[3] ?? 1), mod = +(m[4] ?? 0);
  ...
  function bounds(expr, opt = {}) {
    const p = parse(expr), b = opt.bonus || 0;
    if (opt.mode === 'steady') { const v = p.count * Math.ceil((p.sides + 1) / 2) * p.mul + p.mod + b; return { min: v, max: v, fixed: true }; }
    return { min: p.count * p.mul + p.mod + b, max: p.count * p.sides * p.mul + p.mod + b, fixed: p.count === 0 || p.sides === 1 };
  }
  function roll(expr, state, opt = {}) {
    const p = parse(expr), b = opt.bonus || 0, mode = opt.mode || 'normal', rng = bounds(expr, opt);
    if (mode === 'steady') return { formula: p.formula, total: rng.min, rolls: [], candidates: [rng.min], mode, fixed: true };
    const tries = rng.fixed || mode === 'normal' ? 1 : 2, cands = [];
  ...
  function chance(expr, dc, opt = {}) {
    ...
    for (let i = 0; i < p.count; i++) { const nx = new Map(); for (const [s, w] of sums) for (let f = 1; f <= p.sides; f++) nx.set(s + f, (nx.get(s + f) || 0) + w / p.sides); sums = nx; }
    let pr = 0; for (const [s, w] of sums) if (s * p.mul + p.mod + b >= dc) pr += w;
